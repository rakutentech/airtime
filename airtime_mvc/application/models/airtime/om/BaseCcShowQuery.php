 ...
 * @method     CcShowQuery orderByDbLogo($order = Criteria::ASC) Order by the logo column
 *
 ...
 * @method     CcShowQuery groupByDbLogo() Group by the logo column
 *
 ...
 * @method     CcShow findOneByDbLogo(string $logo) Return the first CcShow filtered by the logo column
 *
 ...
 * @method     array findByDbLogo(string $logo) Return CcShow objects filtered by the logo column
 *
...
abstract class BaseCcShowQuery extends ModelCriteria
{
	 ...
	 * @return    CcShowQuery The current query, for fluid interface
	 */
	public function filterByDbLogo($dbLogo = null, $comparison = null)
	{
		if (null === $comparison) {
			if (is_array($dbLogo)) {
				$comparison = Criteria::IN;
			} elseif (preg_match('/[\%\*]/', $dbLogo)) {
				$dbLogo = str_replace('*', '%', $dbLogo);
				$comparison = Criteria::LIKE;
			}
		}
		return $this->addUsingAlias(CcShowPeer::LOGO, $dbLogo, $comparison);
	}
